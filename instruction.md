...
<!--
1. Документ не следует рассматривать, как финальную версию. Это черновик, который требует дальнейшей доработки.
2. Полужирным выделены неудачные языковые конструкции.
3. В HTML-комментариях отмечены вопросы к ревьюерам, а так же - различные комментарии к тексту.
-->
...
```{note}
Здесь и далее под Windows подразумеваются семейства ОС - Microsoft Windows и Microsoft Windows Server
```
...
Настройка параметров конфигурации драйвера в Windows производится в ПУ (панели управления).
...
<!--
Весь этот раздел нуждается в переработке. Текст ниже был скопирован из исходных заметок "разработчика как есть. Для доработки раздела необходимо прояснить следующие вопросы:

1. Какой набор параметров минимально необходим для настройки файла init<Product>.oraв Linux и в Windows?
2. Какие значения необходимо указывать для описываемых параметров?
3. Существуют ли еще какие-то рекомендации по настройке этого конфига, не упомянутые в исходных материалах?

Текст ниже закомментирован.
-->

<!--
Для настройки файла конфигурации необходимо:

1. Зайти в директорию `C:\Oracle\product\<dbversion>\database\hs\admin\`, где `<dbversion>` - версия Oracle (к примеру, `11.2.0`).
1. Создать файл `init<Product>.ora`, где `<Product>` - имя добавленного в настройках `psqlODBC` источника данных.
...
1. Как перезапустить Oracle Database Listener в Windows и Linux?
2. Есть ли дополнительные рекомендации или информация, которые необходимо отразить в инструкции?
-->
...
<!-- Это предложение нужно сделать лаконичнее -->
...
<!-- Есть ли скриншоты, которые смогут проиллюстрировать этот процесс? -->
...
1. Я не понимаю сути рекомендаций из первоисточника. Что означает "аналогично? О каком контекстном меню идет речь? В Access переиспользуется тот же самый dqy-файл из подраздела выше? Или импорт происходит другим способом, характерным для Access?

2. Есть ли скриншоты, которые смогут проиллюстрировать этот процесс?
-->
